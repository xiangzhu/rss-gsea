...
From the Round 1 null analysis (Section \@ref(load2013-1)), I found that almost 92.5% posterior mass is concentrated on the setting `(h=0.6, theta0=-5)`. In Round 2, I used a finer grid. Specifically, I used the grids `theta0=(-5.25:0.025:-4.75)'` and `h=0.3` when fitting the null model. I estimated the (unnormalized) log importance weights ("pseudo-likelihoods") for all 21 sets of `(h,theta0)` under null. 

```{r,echo=FALSE}
knitr::include_graphics("images/load2013_null_round2.png", dpi=NA)
```
...
```{r, include=FALSE}
null.path <- "~/Dropbox/rss/Data/load_2013/pathway/load2013_null_201609.mat"
...
```{r, include=FALSE}
gsea.path <- "~/Dropbox/rss/Data/load_2013/pathway/load2013_pathway_201609.mat"
...
```{r, include=FALSE}
gsea.path <- "~/Dropbox/rss/Data/load_2013/pathway/load2013_pathway_201604.mat"
...
plotly::plot_ly(data=bf.df, x=round1.log10.bf, y=round2.log10.bf, mode="markers",
                text=paste0("ID: ",id,"; Name: ",name), color=numsnps, size=numgene) %>% add_trace(x=c(0,90), y=c(0,90), mode="lines") %>% add_trace(x=round1.log10.bf, y=fitted(fit), mode="lines") %>% layout(showlegend = F) 
```
...
```{r, include=FALSE}
gtex.path <- "~/Dropbox/rss/Data/gwas_gtex_topexpr/aggregate_results/load2013_gtextop100expr_201609.mat"
...
```{r, include=FALSE}
gtex.path <- "~/Dropbox/rss/Data/gwas_gtexclust/round2_results/load2013_gtexclust_201609.mat"
gtex.df <- gtex.mat2df.round2(gtex.path) 
```
