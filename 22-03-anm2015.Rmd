...
From the Round 1 null analysis (Section \@ref(anm2015-1)), I found that almost 99.9% posterior mass is concentrated on the setting `(h=0.4, theta0=-5.5)`. In Round 2, I used a finer grid. Specifically, I used the grids `theta0=(-5.75:0.025:-5.25)'` and `h=0.4` when fitting the null model. I estimated the (unnormalized) log importance weights ("pseudo-likelihoods") for all 21 sets of `(h,theta0)` under null. 

```{r,echo=FALSE}
knitr::include_graphics("images/anm2015_null_round2.png", dpi=NA)
```
...
```{r, include=FALSE}
null.path <- "~/Dropbox/rss/Data/anm_2015/pathway/anm2015_null_201609.mat"
...
```{r, include=FALSE}
gsea.path <- "~/Dropbox/rss/Data/anm_2015/pathway/anm2015_pathway_201609.mat"
...
plotly::plot_ly(data=bf.df, x=round1.log10.bf, y=round2.log10.bf, mode="markers",
                text=paste0("ID: ",id,"; Name: ",name), color=numsnps, size=numgene) %>% add_trace(x=c(0,1100), y=c(0,1100), mode="lines") %>% add_trace(x=round1.log10.bf, y=fitted(fit), mode="lines") %>% layout(showlegend=F) 
```
