...
I used the grids `theta0 = (-4:0.25:-1)'` and `h = (0.3:0.1:0.6)'` when fitting the null model.

Here are the (unnormalized) log importance weights ("pseudo-likelihoods") for all 52 sets of `(h,theta0)` under null.
```
TBA
```
...
```{r, echo=FALSE}
DT::datatable(null.df,rownames=FALSE,class="hover") %>% DT::formatRound(c("logw.step1","logw.step2","posp.step1","posp.step2"), 2) %>% DT::formatRound(c("posp.step1","posp.step2"), 4)
```

### Fit the enrichment model

To perform GSEA, I set `(h=0.3, theta0=-2)`, since the null analysis suggests that almost 100% posterior mass is placed on this setting. For the log-fold enrichment parameter, I use the grid `theta  = (0:0.01:2)'`. 
```{r, include=FALSE}
...
```{r, echo=FALSE}
DT::datatable(gsea.df,rownames=FALSE,class="hover") %>% DT::formatRound(c("log10.bf","theta.95lb","theta.95ub"), 2) %>% DT::formatRound(c("theta.mean"), 3) %>% DT::formatStyle("name",fontSize='9ptx')
```
