...
From the Round 1 null analysis (Section \@ref(mch2012-1)), I found that almost 100% posterior mass is concentrated on the setting `(h=0.6, theta0=-4)`. In Round 2, I used a finer grid. Specifically, I used the grids `theta0=(-4.75:0.05:-3.75)'` and `h=0.6` when fitting the null model. I estimated the (unnormalized) log importance weights ("pseudo-likelihoods") for all 21 sets of `(h,theta0)` under null.

```{r,echo=FALSE}
knitr::include_graphics("images/mch2012_null_round2.png", dpi=NA)
```
...
```{r, include=FALSE}
null.path <- "~/Dropbox/rss/Data/mch_2012/pathway/mch2012_null_201609.mat"
...
plotly::plot_ly(data=bf.df, x=round1.log10.bf, y=round2.log10.bf, mode="markers",
                text=paste0("ID: ",id,"; Name: ",name), color=numsnps, size=numgene) %>% add_trace(x=c(0,2400), y=c(0,2400), mode="lines") %>% add_trace(x=round1.log10.bf, y=fitted(fit), mode="lines") %>% layout(showlegend=F) 
```
